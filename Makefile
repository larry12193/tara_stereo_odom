...
OPENCV_LIBS=-L $(OPENCV_INSTALL_PREFIX)/lib -lopencv_core -lopencv_imgproc -lopencv_highgui -lopencv_videoio -lopencv_ximgproc -lopencv_xfeatures2d -lopencv_features2d -lopencv_calib3d


#Building Targets
default: $(OUTPUT)
 
$(OUTPUT): stereoStream.cpp common_libs 
	@echo "\n${BLUE}${BOLD}Building $(OUTPUT) Application${NC}"
	@$(CC) $< -o $@ $(CFLAGS) $(ECON_LIBS) $(OPENCV_LIBS) $(CUSTOM_LIBS)
