...
int main (int argc, char** argv) {
  ...
  param.calib.f  = 719.3978; // focal length in pixels
  param.calib.cu = 384.0452; // principal point (u-coordinate) in pixels
  param.calib.cv = 250.2095; // principal point (v-coordinate) in pixels
  param.base     = 0.059867; // baseline in meters
  ...
  for (int32_t i=1; i<=584; i++) {

    // input file names
    char base_name[256]; sprintf(base_name,"%06d.png",i);
    string left_img_file_name  = dir + "/L_" + base_name;
    string right_img_file_name = dir + "/R_" + base_name;
